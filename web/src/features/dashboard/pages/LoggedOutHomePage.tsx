...
import CommunityDirectory from "../../community/components/CommunityDirectory";

export default function LoggedOutHomePage() {
  const { data: discovered } = useQuery(discoveryQuery());
  const dids = discovered?.map((bbs) => bbs.did) ?? [];
  const { data: pinCounts } = useQuery(pinCountsQuery(dids));
  const suggestions = discovered?.map(bbsToSuggestion);
  const rankedByPins =
